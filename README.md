# Quantum Mechanics for Organic Electronics

**A 12-Lecture Series**

Interactive lecture slides for a graduate-level quantum mechanics course, with a focus on organic semiconductor physics.

*Zesheng Liu · Linköping University · Laboratory of Organic Electronics*

> **Understanding over derivation. History over formalism. Animation over equation.**

## Lecture Map

| # | QM Core | OE Connection | Status |
|---|---------|---------------|--------|
| 01 | [History of QM](https://zeshengliu213.github.io/quantum-mechanics-lectures/lecture01/index.html) | Why classical physics fails for π-electrons | ✅ Complete |
| 02 | [From Matter Waves to Schrödinger](https://zeshengliu213.github.io/quantum-mechanics-lectures/lecture02/index.html) | Wave equation for quantum particles | ✅ Complete |
| 03 | Particle in a Box | Conjugation length vs. bandgap | 📝 Planned |
| 04 | Quantum Harmonic Oscillator | Franck-Condon, Huang-Rhys, vibronic spectra | 📝 Planned |
| 05 | Tunneling | Charge injection at metal/organic interfaces, SAMs | 📝 Planned |
| 06 | Angular Momentum & H Atom | Carbon atomic orbitals → sp² hybridization | 📝 Planned |
| 07 | Spin & Identical Particles | Singlet/triplet, OLED spin statistics, radicals | 📝 Planned |
| 08 | Perturbation Theory | Intermolecular coupling, CT states, interface dipoles | 📝 Planned |
| 09 | Variational Method | LCAO, Hückel theory → DFT bridge | 📝 Planned |
| 10 | Periodic Potentials & Bands | Band vs. hopping transport in organic solids | 📝 Planned |
| 11 | Electron-Phonon Coupling | Polarons, Marcus theory, reorganization energy | 📝 Planned |
| 12 | Electron Transfer at Interfaces | D/A charge transfer, molecular conduction | 📝 Planned |
| 13 | Excitons & Many-Body Physics | Frenkel exciton, exciton diffusion, device limits | 📝 Planned |

## Features

- Browser-based slide deck with keyboard/touch navigation
- Bilingual support (中文/English) with language toggle
- Interactive canvas animations and visualizations
- KaTeX math rendering
- Fullscreen support (press `F`)

### Lecture 01 Animations
- Blackbody radiation spectrum (adjustable temperature)
- Photoelectric effect (frequency & intensity controls)
- Double-slit electron interference (real-time accumulation)
- Compton scattering (adjustable angle)
- Stern-Gerlach experiment (single & cascaded)
- Uncertainty principle visualization

### Lecture 02 Animations
- Classical trajectory vs. double-slit electron interference
- Uncertainty principle (position–momentum Gaussian pair)
- Two-state superposition interference

## Navigation

| Key | Action |
|-----|--------|
| `Arrow Right` / `Space` | Next slide |
| `Arrow Left` | Previous slide |
| `F` | Toggle fullscreen |
| Click / Swipe | Navigate |

## Primary References

- K&B: Köhler & Bässler, *Electronic Processes in Organic Semiconductors* (Wiley, 2015)
- Nitzan: *Chemical Dynamics in Condensed Phases* (Oxford, 2006)
- 顾樵:《量子力学 I》

## Author

Zesheng Liu · Linköping University
